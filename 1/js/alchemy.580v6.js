...
function WorkspaceBox(a, b, c) {
    "string" == typeof a || "number" == typeof a
        ? ((this.id = parseInt(a, 10)), (this.$el = $(workspace.el.appendChild(this.createElement(b)))), this.init(c))
        : ((this.id = a.getAttribute("data-elementid")), (this.$el = $(a)), this.changeType(), this.init()),
        this.initEvents();
}
var game = {
        platform: "web",
        ...
        initProgress: function () {
            ...
            for (
                game.history = JSON.parse(storage.getHistory()),
                    ...
                    a = 0,
                    b = game.history.parents.length;
                a < b;
                a++
            )
                for (c = workspace.sex(game.history.parents[a]), d = 0; d < c.length; d++)
                    bases.base[c[d]].hasOwnProperty("hidden")
                        ? game.hiddenElements.push(c[d])
                        : game.progress.indexOf(c[d]) === -1 &&
                          game.prime.indexOf(c[d]) === -1 &&
                          (game.progress.push(c[d]), game.checkIfFinal(c[d]) || game.progressWithoutFinal.push(c[d]));
        ...
        onChildCreated: function () {
            $(document).on("childCreated", function (a, b, c) {
                ...
                for (d = 0; d < b.length; d++)
                    game.checkIfNotAlreadyDone(c) &&
                        ((e = new Date().getTime()),
                        game.history.parents.push(c),
                        game.history.date.push(e),
                        $(document).trigger("updateHistory", [c, e]),
                        bases.base[b[d]].hasOwnProperty("hidden") &&
                            ($(document).trigger("hiddenElementCreated", [b[d]]),
                            game.hiddenElements.indexOf(b[d]) === -1 && game.hiddenElements.push(b[d]))),
                        game.progress.indexOf(b[d]) !== -1 ||
                            game.prime.indexOf(b[d]) !== -1 ||
                            bases.base[b[d]].hasOwnProperty("hidden") ||
                            (game.progress.push(b[d]),
                            game.checkIfFinal(b[d])
                                ? game.finalElements.push(b[d])
                                : game.progressWithoutFinal.push(b[d]),
                            game.changeProgressCounter(),
                            $(document).trigger("newChildCreated", [b[d]]));
            });
        },
        ...
        checkStructure: function () {
            ...
            for (a in bases.base) {
                ...
                bases.base.hasOwnProperty(a) &&
                    (bases.base[a].hasOwnProperty("prime") && game.prime.push(parseInt(a, 10)),
                    bases.base[a].hasOwnProperty("hidden") || game.maxProgress++);
            }
        },
        getFinalElements: function () {
            var a, b;
            for (a = 0, b = game.progress.length; a < b; a++)
                game.checkIfFinal(game.progress[a]) &&
                    game.finalElements.indexOf(game.progress[a]) === -1 &&
                    game.finalElements.push(game.progress[a]);
        },
        checkIfFinal: function (a) {
            var b,
                c,
                d,
                e = Object.keys(bases.base);
        ...
        checkMaxConnections: function (a) {
            ...
            var b;
            for (b in bases.base)
                bases.base[b].hasOwnProperty("parents") && (game.maxConnections += bases.base[b].parents.length);
        },
        checkIfNotAlreadyDone: function (a) {
            for (
                var b = [Math.min(a[0], a[1]), Math.max(a[0], a[1])], c = 0, d = game.history.parents.length;
                c < d;
                ++c
            )
                if (
                    Math.min(game.history.parents[c][0], game.history.parents[c][1]) === b[0] &&
                    Math.max(game.history.parents[c][0], game.history.parents[c][1]) === b[1]
                )
                    return !1;
            return !0;
        },
        initEvents: function () {
            ...
            void 0 === a && $("#toggleFullscreen").hide(),
        ...
        saveOldProgress: function (a, b) {
            ...
            var d = 1e3;
            c.parents.length > d && (c.parents.splice(0, c.parents.length - d), c.date.splice(0, c.date.length - d)),
                window.storage.setOldHistory(c);
        },
        checkHistoryIntegrity: function (a) {
            var b,
                ...
                g = function (a) {
                    return !(
                        !bases.base.hasOwnProperty(a) ||
                        (!bases.base[a].hasOwnProperty("prime") && f.indexOf(a) === -1)
                    );
                };
            ...
            for (b = a.parents.length - 1; b >= 0; b--)
                if (!g(a.parents[b][0]) || !g(a.parents[b][1])) {
                    for (console.log(a.parents[b]), d = workspace.sex(a.parents[b]), e = 0; e < d.length; e++)
                        f.splice(f.indexOf(d[e]), 1);
        ...
        reportError: function (a, b, c, d, e) {
            ...
            return $.ajax({ type: "POST", url: "php/errorReporting.php", data: { data: JSON.stringify(f) } }), !1;
        },
    ...
    bases = {
        ...
        load: function () {
            bases.initEvents(),
                            ...
                            onprogress: function (a) {
                                a.lengthComputable &&
                                    $(document).trigger("baseLoadingProgress", [
                                        parseInt((a.loaded / a.total) * 100, 10),
                                    ]);
                            },
                        },
                    }),
                ...
                fixedGetJson("resources/images.580.json", function (a) {
                    (bases.images = a), (bases.imagesLoaded = !0), $(document).trigger("imagesLoaded");
                });
        },
        initEvents: function () {
            $(document).on("languageChanged", function () {
                fixedGetJson(localization.getURL("names.580.json"), function (a) {
    ...
    loadingScreen = {
        transition: { PATH_TIME: 0.5, OPACITY_DELAY: 0.1, OPACITY_TIME: 0.75, PROGRESS_TIME: 0.3 },
        progress: 0,
        shownProgress: 0,
        progressList: {
        ...
        initPath: function () {
            if (
                ...
                navigator.userAgent.toLowerCase().indexOf("firefox") > -1 &&
                    ...
                    (loadingScreen.path.style.strokeDashoffset = Math.round(a / 12) + "px"),
        ...
        completedLoading: function () {
            window.setTimeout(
                ...
                1e3 * (loadingScreen.transition.PATH_TIME + loadingScreen.transition.OPACITY_DELAY)
            ),
        ...
        changeProgress: function () {
            ...
            if (loadingScreen.progress > loadingScreen.shownProgress) {
                ...
                loadingScreen.progress - loadingScreen.shownProgress > 20 &&
                    ...
                    (a = 2 * Math.floor((loadingScreen.progress - loadingScreen.shownProgress) / 4)),
                    (loadingScreen.shownProgress += a),
            ...
            if (
                ...
                navigator.oscpu.toLowerCase().indexOf("windows nt 6.1") > -1
                    ? (loadingScreen.path.style.strokeDashoffset =
                          Math.round((a - (loadingScreen.progress / 100) * a) / 12) + "px")
                    : (loadingScreen.path.style.strokeDashoffset = a - (loadingScreen.progress / 100) * a);
            }
        },
        ...
        initEvents: function () {
            for (var a in loadingScreen.progressList)
                $(document).on(a, function (a, b) {
                    loadingScreen.progressList[a.type].hasOwnProperty("incremental")
                        ? loadingScreen.progressList[a.type].current < 100 &&
                          ((loadingScreen.progress -= Math.round(
                              (loadingScreen.progressList[a.type].current / 100) *
                                  loadingScreen.progressList[a.type].value
                          )),
                          (loadingScreen.progress += Math.round((b / 100) * loadingScreen.progressList[a.type].value)),
                          (loadingScreen.progressList[a.type].current = b))
                        : ("basesLoaded" === a.type
                              ? ((loadingScreen.progress +=
                                    (loadingScreen.progressList[a.type].value *
                                        (200 -
                                            loadingScreen.progressList.namesLoadingProgress.current -
                                            loadingScreen.progressList.baseLoadingProgress.current)) /
                                    100),
                                (loadingScreen.progressList.namesLoadingProgress.current = 100),
                                (loadingScreen.progressList.baseLoadingProgress.current = 100))
                              : loadingScreen.incrementProgress(a.type),
                          $(document).off(a.type),
                          loadingScreen.list.push(a.type));
                });
        },
        hide: function () {
            ...
            (d.style[window.getStyleProperty("transition")] = "margin-top 0.75s ease-in"),
                ...
                (d.style["margin-top"] = window.innerHeight + "px"),
                window.setTimeout(function () {
                    (d.style["margin-top"] = $(d).css("margin-top") + "px"),
                        void 0 !== a
                            ? (loadingScreen.el.style[a] = c(window.innerHeight))
        ...
        messages: {
            ...
            choose: function () {
                var a,
                    b = loadingScreen.messages.getLevel(),
                    c = [];
                for (i = 0; i < loadingScreen.messages.list[b].length; i++)
                    loadingScreen.messages.shown.indexOf(loadingScreen.messages.list[b][i].id) === -1 &&
                        c.push(loadingScreen.messages.list[b][i]);
                if (1 === c.length) a = c[0].id;
                else if (loadingScreen.messages.list[b][0].hasOwnProperty("P"))
                    a = loadingScreen.messages.probabilityChoose(b);
                else {
            ...
            probabilityChoose: function (a) {
                for (var b = 0, c = 0, d = loadingScreen.messages.list[a].length; c < d; c++)
                    b += loadingScreen.messages.list[a][c].P;
    ...
    settings = {
        ...
        initContent: function () {
            settings.offContent(),
                ...
                $("#settingsResetProgress").on("click", function (a) {
                ...
                $(document).on("change", "#settingsMarkFinalElements", function () {
                    (window.settings.data.markFinalElements = this.checked),
                        ...
                        window.settings.data.markFinalElements
                            ? workspace.$el.find(".element").each(function (a, b) {
                                  game.finalElements.indexOf(parseInt(b.getAttribute("data-elementId"), 10)) !== -1 &&
                                      (b.className += " finalElement");
                              })
                ...
                $(document).on("change", "#settingsHideFinalElements", function () {
                    (window.settings.data.hideFinalElements = this.checked),
                        window.settings.data.hideFinalElements ? window.library.refresh() : window.library.reload(),
                        $(document).trigger("updateSettings");
                }),
            ...
            $(document).on("playerLoaded", a),
                $(document).on("loggedOut", function () {
                    $("#loggedInAs").hide();
                }),
                "undefined" != typeof GoogleAPI.player.name && "" !== GoogleAPI.player.name && a(),
    ...
    localization = {
        ...
        changeLanguage: function (a) {
            a !== localization.language &&
                (localization.setLanguage(a), localization.loadResources(), $(document).trigger("languageChanged"));
        },
        setLanguage: function (a) {
            localization.languages.hasOwnProperty(a) || (a = "en"),
        ...
        loadResources: function () {
            fixedGetJson(localization.getURL("languagePack.580.json"), function (a) {
                (localization.data = a), (localization.loaded = !0), $(document).trigger("languagePackLoaded");
            });
        },
        get: function (a) {
            var b = a.split("-");
            return localization.data.hasOwnProperty(b[0]) && localization.data[b[0]].hasOwnProperty(b[1])
                ? localization.data[b[0]][b[1]]
                : "";
        },
        getURL: function (a) {
            ...
            b.open("HEAD", a, !0),
                (b.onreadystatechange = function (c) {
                    this.readyState === this.DONE &&
                        (200 === this.status
                            ? loading.analyzeModificationDate(a, b.getResponseHeader("Last-Modified"))
                            : (delete loading.modificationDates[a], storage.updateModificationDates()));
                }),
                b.send();
        },
        getURL: function (a) {
            return game.isOnline && loading.modificationDates.hasOwnProperty(a)
                ? a + "?t=" + loading.modificationDates[a]
                : a;
        },
        analyzeModificationDate: function (a, b) {
            if (null !== b) {
                var c = loading.convertToMilisecondsDate(b);
                (!loading.modificationDates.hasOwnProperty(a) || loading.modificationDates[a] < c) &&
                    ((loading.modificationDates[a] = c), storage.updateModificationDates());
            }
        },
    ...
    GoogleAPI = {
        ...
        handleAuth: function (a) {
            (GoogleAPI.checked = !0),
                !a || a.error || GoogleAPI.logged
                    ? GoogleAPI.logged
                        ? $(document).trigger("loggedOut")
                        : $(document).trigger("notLoggedIn")
                    : ((GoogleAPI.logged = !0),
                      (GoogleAPI.accessToken = a.access_token),
                      storage.setAuthUser(a.authuser),
        ...
        loadPlayer: function () {
            var a = gapi.client.games.players.get({ playerId: "me" });
            a.execute(function (a) {
                a.error ||
                    ((GoogleAPI.player.id = a.playerId),
                    (GoogleAPI.player.name = a.displayName),
    ...
    gestures = {
        ...
        init: function () {
            for (var a = 0; a < gestures.startEvents.length; a++)
                document.addEventListener(gestures.startEvents[a], function (a) {
        ...
        longPress: {
            ...
            down: function (a) {
                clearTimeout(gestures.longPress.timer),
                    document.addEventListener(gestures.events[a.type][0], gestures.longPress.move),
                    (a = "touchstart" === a.type ? a.changedTouches[0] : a),
                    (gestures.longPress.initPosition = { x: a.pageX, y: a.pageY }),
                    (gestures.longPress.target = a.target),
            ...
            move: function (a) {
                if (gestures.longPress.initPosition.x !== -1 && gestures.longPress.initPosition.y !== -1) {
                    ...
                    ((c && d > gestures.longPress.WIGGLE_THRESHOLD) || a.target !== gestures.longPress.target) &&
                        (clearTimeout(gestures.longPress.timer),
            ...
            up: function (a) {
                clearTimeout(gestures.longPress.timer),
                    document.removeEventListener(
                        gestures.events[gestures.endEvents[a.type]][0],
...
window.storage = {
    ...
    checkOldVersionHistory: function () {
        if (
            ...
            for (
                ...
                b.parents.push([
                    Math.min(a.parents[c][0] + 1, a.parents[c][1] + 1),
                    Math.max(a.parents[c][0] + 1, a.parents[c][1] + 1),
                ]),
                    b.date.push(a.date[c]);
            window.localStorage.setItem("progress", JSON.stringify(b)),
                bases.base && (game.initProgress(), game.getFinalElements(), library.reload()),
    ...
    checkImportedHistory: function () {
        if (null !== window.localStorage.getItem("history") && null !== window.localStorage.getItem("progress")) {
            for (var a, b = JSON.parse(window.localStorage.getItem("history")), c = 0, d = b.parents.length; c < d; c++)
                (a = [
                    Math.min(b.parents[c][0] + 1, b.parents[c][1] + 1),
                    Math.max(b.parents[c][0] + 1, b.parents[c][1] + 1),
                ]),
                    game.checkIfNotAlreadyDone(a) && (game.history.parents.push(a), game.history.date.push(b.date[c]));
            $(document).trigger("updateHistory"),
                localStorage.removeItem("history"),
                bases.base &&
                    (game.initProgress(),
                    game.getFinalElements(),
                    "undefined" != typeof library.el && null !== library.el && library.reload());
        }
    },
    ...
    checkSettings: function () {
        (null !== window.localStorage.getItem("settings") && 0 !== window.localStorage.getItem("settings").length) ||
    ...
    checkReset: function () {
        (null !== window.localStorage.getItem("reset") && 0 !== window.localStorage.getItem("reset").length) ||
...
var search = {
        ...
        initAlphabet: function () {
            search.$alphabet.on("click", "li", function () {
                var a = $(this);
                if (((search.activeLetter = a.text().toLowerCase()), null === search.activeCategory))
            ...
            var a = document.getElementById("searchBar"),
                b = function () {
                    $(document).trigger("searchOccured"),
                        "text" === a.type && ((a.value = ""), (a.type = "hidden"), library.refresh());
                };
            $(document).keydown(function (b) {
                if (
                    (menu.$el.is(":visible") ||
                        (0 != $(":focus").length && $(":focus")[0] !== a && "INPUT" === $(":focus")[0].tagName) ||
                        (((b.keyCode >= 65 && b.keyCode <= 90) || 32 == b.keyCode) &&
                            (!b.ctrlKey || (b.ctrlKey && b.altKey)) &&
                            ((a.type = "text"),
                            a.focus(),
                            "undefined" != typeof iscrollLibrary ? iscrollLibrary.scrollTo(0, 0, 300) : initIScroll())),
                    8 === b.keyCode)
                ) {
                    var c = !1,
                ...
                (a.onkeyup = function (c) {
                    "text" !== a.type || (0 !== a.value.length && 27 !== c.keyCode)
                ...
                (a.onblur = b),
                $(document).on("libraryBoxDraggingStart workspaceBoxDraggingStart", b);
        },
        scrollToLetter: function (a, b) {
            if (
                ("undefined" == typeof iscrollLibrary && initIScroll(),
                "undefined" != typeof iscrollLibrary && iscrollLibrary.maxScrollY < 0)
            ) {
                var c = Math.max(
                    -library.elementOuterHeight * search.getNoElementsBefore(a, b),
    ...
    update = {
        types: {
            history: {
                ...
                lastSaveVersion: "",
                data: function () {
                    var a = { parents: game.history.parents, date: game.history.date, reset: storage.getReset() };
        ...
        synchronize: function (a) {
            update.types[a]["synchronized"] ||
                update.load(a, function () {
        ...
        load: function (a, b) {
            gapi.client.request({
                ...
                callback: function (c, d) {
                    var e = JSON.parse(d);
                    404 == e.gapiRequest.data.status
                        ? ((update.types[a].lastSaveVersion = c.currentStateVersion), update.save(a))
                        : 401 == e.gapiRequest.data.status
                          ? $(document).trigger("unauthorized")
                          : "appstate#getResponse" == c.kind &&
                            c.hasOwnProperty("data") &&
                            ((update.types[a].lastSaveVersion = c.currentStateVersion),
                            update.types[a].merge(JSON.parse(c.data))),
                        null != b && b();
                },
            });
        },
        save: function (a, b) {
            var c = {};
            "" != update.types[a].lastSaveVersion && (c.currentStateVersion = update.types[a].lastSaveVersion),
                gapi.client.request({
                    path: update.path + update.types[a].slot,
                    params: c,
                    body: { kind: "appstate#updateRequest", data: update.types[a].data() },
                    method: "put",
                    callback: function (c, d) {
                        var e = JSON.parse(d);
                        409 == e.gapiRequest.data.status
                            ? update.load(a, function () {
                                  update.save(a);
                              })
                            : 401 == e.gapiRequest.data.status
                              ? $(document).trigger("unauthorized")
                              : "appstate#writeResult" == c.kind &&
                                (c.hasOwnProperty("error") ||
                                    ((update.types[a].lastSaveVersion = c.currentStateVersion), null != b && b()));
                    },
                });
        },
        mergeHistoryData: function (a) {
            var b = function () {
                    if (a.reset !== -1) {
                        for (var b = [], d = [], e = game.history.parents.length - 1; e >= 0; e--)
                            parseInt(game.history.date[e], 10) < a.reset &&
                                (b.push(game.history.parents.splice(e, 1)[0]),
                                d.push(game.history.date.splice(e, 1)[0]),
                                (c = !0));
            ...
            if (a.parents.length > 0)
                if (game.history && 0 === game.history.parents.length)
                    storage.setReset(a.reset), delete a.reset, (game.history = a), (c = !0);
                else {
                    for (var d = 0, e = a.parents.length; d < e; d++)
                        game.checkIfNotAlreadyDone(a.parents[d]) &&
                            (game.history.parents.push(a.parents[d]), game.history.date.push(a.date[d]), (c = !0));
                    b();
                }
            else b();
            c &&
    ...
    library = {
        ...
        init: function () {
            (library.elContainter = document.createDocumentFragment()),
                ...
                (library.droppable = new Droppable(library.outerLibrary, { tolerance: "touch" })),
                library.$outerLibrary.on("droppableDrop", function (a, b) {
                    "workspaceBox" === b.element.getAttribute("data-elementtype")
                        ? workspace.del(b.$element.data("ptr"))
                        : $(b.helper).off().data("ptr", null).remove();
                }),
            ...
            var a = 5,
                b = window.setTimeout(function () {
                    library.addAll(),
                        loadingScreen.list.indexOf("historySynchronized") === -1 &&
                            loadingScreen.list.indexOf("notLoggedIn") === -1 &&
        ...
        initOnScrollAction: function () {
            ...
            iscrollLibrary.on("scrollStart", function () {
                ...
                a.on(gestures.startEvents.join(" "), function (a) {
                    ...
                    var b,
                        c = Math.floor((a.offsetY + -1 * iscrollLibrary.y) / library.elementOuterHeight);
                    (b =
                        settings.data.hideFinalElements &&
                        library.el.children.length !== game.progressWithoutFinal.length + game.prime.length
                            ? library.el.querySelector('.element[data-elementid="' + library.sortedProgress[c] + '"]')
                            : library.el.children[c]),
                        $(b).data("ptr").draggable.dragStart(a, a);
                });
        },
        add: function (a) {
            new LibraryBox(a);
        ...
        addOne: function (a) {
            ...
            var f = settings.data.hideFinalElements
        ...
        addAll: function () {
            ...
            library.el.appendChild(library.elContainter),
                ...
                library.refreshIscroll(d.length),
                $(document).trigger("libraryShowed");
        },
        clear: function () {
            for (var a, b, c = library.el.getElementsByClassName("element"), d = 0, e = c.length; d < e; d++)
                (a = $(c[d])),
                    (b = a.data("ptr")),
                    b.draggable && b.draggable.destroy(),
                    (b.draggable = null),
                    a.find("img").off(),
                    a.data("ptr", null).off();
        ...
        hideAll: function () {
            for (var a, b, c = library.el.getElementsByClassName("element"), a = 0, b = c.length; a < b; a++)
        ...
        show: function (a) {
            var b,
                ...
                f,
                g,
                h = library.el.getElementsByClassName("element"),
                i = a || game.prime.concat(game.progress);
            for (
                i = library.sort(i, !0),
                    f = i.length,
                    "undefined" == typeof a && (library.sortedProgress = []),
                    g = 0,
                    b = 0,
                    c = h.length;
                b < c;
                ...
                (d = h[b].getAttribute("data-elementId")),
                    (e = i.indexOf(parseInt(d, 10))),
                    !library.checkIfHideFinal(parseInt(d, 10)) && e > -1
                        ? ((h[b].style.display = "block"),
                          i.splice(e, 1),
                          "undefined" == typeof a && library.sortedProgress.push(d),
                          g++)
        ...
        reloadImages: function () {
            var a,
                b,
                c,
                d = library.el.getElementsByClassName("element"),
                e = function (a) {
                    var b = $(this);
                    b.data("ptr").draggable.changeHandles("img"), b.off("dragEnd", e);
                };
            for (a = 0, b = d.length; a < b; a++)
                d[a].firstChild.nextSibling.src =
                    "data:image/png;base64," + bases.images[d[a].getAttribute("data-elementId")];
            for (a = 0, b = d.length; a < b; a++)
                d[a].firstChild.parentNode.removeChild(d[a].firstChild),
                    (d[a].firstChild.style.display = "block"),
                    (c = $(d[a]).data("ptr").draggable),
                    c.isDragging ? $(d[a]).on("dragEnd", e) : c.changeHandles("img");
        },
        markFinalElements: function () {
            var a, b, c;
            if (window.settings.data.markFinalElements && !window.settings.data.hideFinalElements)
                for (c = game.finalElements, a = 0, b = c.length; a < b; a++)
                    library.el.querySelector('.element[data-elementid="' + c[a] + '"]').className += " finalElement";
            else
                for (c = library.el.getElementsByClassName("finalElement"); c.length > 0; )
                    c[0].className = c[0].className.replace(/\bfinalElement\b/, "");
        },
        ...
        showCategory: function (a) {
            ...
            for (b = 0, c = f.length; b < c; b++)
                if (bases.base[f[b]].tags)
                    for (d = 0; d < bases.base[f[b]].tags.length; d++)
                        bases.base[f[b]].tags[d] === a && e.push(parseInt(f[b], 10));
            return e;
        },
        showLetterAndCategory: function (a, b) {
            var c,
                d,
                e = [];
            for (b = library.showCategory(b), c = 0, d = b.length; c < d; c++)
                bases.names[b[c]][0] === a && e.push(b[c]);
            return e;
        },
        showPhrase: function (a) {
            for (
                var b, c, d = [], e = "[]{}'/\\()?<>+.*^$", f = game.prime.concat(game.progress), b = 0;
                b < e.length;
                b++
            )
                a = a.split(e[b]).join("");
        ...
        sort: function (a, b) {
            ...
            return (
                "alphabetically" === library.sortMode
                    ? (d = library.sortAlphabetically(c))
                    : "time" === library.sortMode && (d = library.sortByTime(c)),
                b ? d : void library.show(d)
            );
        },
        sortAlphabetically: function (a) {
            ...
            for (
                "en" !== localization.language ? f.sort(localization.compare) : f.sort(), b = 0, c = f.length;
                b < c;
                b++
            )
                g.push(e[f[b]]);
        ...
        refreshIscroll: function (a) {
            if (iscrollLibrary) {
                ...
                b < a * library.elementOuterHeight
                    ? iscrollLibrary.refresh(!0, a * library.elementOuterHeight + 50)
                    : iscrollLibrary.refresh(!0, b);
            }
        },
    };
window.addEventListener("orientationChange", library.refreshIScroll, !1),
    (LibraryBox.prototype.createElement = function (a) {
        var b = document.createElement("div");
        b.setAttribute("class", "element"),
            b.setAttribute("data-elementType", "libraryBox"),
            b.setAttribute("data-elementId", this.id),
            (b.style.display = a ? "none" : b.style.display),
            game.finalElements.indexOf(this.id) !== -1 &&
        ...
        bases.imagesLoaded
            ? ((c = document.createElement("img")),
              (c.style.opacity = 0),
              (c.src = "data:image/png;base64," + bases.images[this.id]),
              (c.alt = bases.names[this.id]),
              (c.onload = function () {
                  (this.style.opacity = 1), (c.onload = null);
              }))
            : a
              ? ((c = document.createElement("img")),
                (c.src = "data:image/png;base64," + library.loadingImage),
                (c.alt = bases.names[this.id]))
              : ((c = document.createElement("img")),
                (c.style.display = "none"),
                (c.alt = bases.names[this.id]),
                (d = document.createElement("span")),
                (d.className += " elementLoadingImage"));
        var e = document.createElement("div");
        return (
            e.setAttribute("class", "elementName"),
            (e.textContent = bases.names[this.id]),
            d && b.appendChild(d),
            b.appendChild(c),
            b.appendChild(e),
            b
    ...
    (LibraryBox.prototype.initDraggable = function () {
        ...
        (this.draggable = new Draggable(this.$el[0], {
            ...
            this.$el.on("dragEnd", function (a, b) {
                b.isOver || (new WorkspaceBox(b.helper), $(document).trigger("elementDropped")),
                    iscrollLibrary && ((iscrollLibrary.initiated = !1), iscrollLibrary.enable());
            });
    }),
...
var workspace = {
    init: function () {
        (workspace.el = document.getElementById("workspace")),
            ...
            $(document).on("draggableDroppedFix", function (a, b) {
                new WorkspaceBox(b.helper);
            }),
            window.addEventListener("orientationchange", workspace.recalculateElements, !1),
    ...
    clone: function (a, b) {
        ...
        d.initMouseEvent("mousedown", !0, !0, window, 1, a.pageX, a.pageY, a.pageX, a.pageY, !1, !1, !1, !1, 0, null),
            c.$el[0].dispatchEvent(d),
            c.droppable.startListeningToDrag(b.draggable);
    },
    clear: function () {
        settings.data.markFinalElements && workspace.$el.find(".finalElement").length > 0
    ...
    clearSpecified: function (a) {
        for (var b, c, d = 0, e = a.length; d < e; d++)
            (b = $(a[d])),
                (c = b.data("ptr")),
                c &&
                    (c.droppable && c.droppable.destroy(),
                    c.draggable && c.draggable.destroy(),
                    (c.draggable = null),
                    (c.droppable = null)),
                b.find("img").off(),
                b.data("ptr", null).off().remove();
    },
    del: function (a) {
        (a.$el[0].style.display = "none"),
            $(document).off(gestures.startEvents.join(" ")),
            $(a.$el).off(gestures.startEvents.join(" ")),
            a.droppable && a.droppable.destroy(),
            a.draggable && a.draggable.destroy(),
            (a.draggable = null),
            (a.droppable = null),
            a.$el.find("img").off(),
            a.$el.data("ptr", null).off().remove(),
            (a = null);
    },
    reloadImages: function () {
        for (var a, b = workspace.el.getElementsByClassName("element"), c = 0; c < b.length; c++)
            (a = b[c].firstChild),
                (a.src = "data:image/png;base64," + bases.images[b[c].getAttribute("data-elementId")]);
    },
    reloadNames: function () {
        for (var a, b, c = workspace.el.getElementsByClassName("element"), d = 0; d < c.length; d++)
            (a = $(c[d])), (b = a.find("div")), (b[0].innerHTML = bases.names[a.attr("data-elementId")]);
    },
    sex: function (a) {
        var b,
            c,
    ...
    ifSex: function (a, b) {
        ...
        if (bases.base[b].hasOwnProperty("parents"))
            for (c = 0; c < bases.base[b].parents.length; c++)
                Math.min(a[0], a[1]) === Math.min(bases.base[b].parents[c][0], bases.base[b].parents[c][1]) &&
                    Math.max(a[0], a[1]) === Math.max(bases.base[b].parents[c][0], bases.base[b].parents[c][1]) &&
                    (d += 1);
        return d;
    },
        ...
        var d,
            e = function (c) {
                var d = (a[0].top - a[1].top) / (a[0].left - a[1].left),
                    e = 0.35 * b,
                    f = { x: (a[0].left + a[1].left) / 2, y: (a[0].top + a[1].top) / 2 },
    ...
    hideUnderLibrary: function () {
        for (
            var a = workspace.$el.find(".element"),
                b = workspace.$el.width() - $("#side").width(),
                c = $(".element").width(),
                d = 0;
            d < a.length;
            d++
            ...
            var e = $(a[d]);
            e.offset().left + c / 2 > b
                ? ((a[d].style.visibility = "hidden"), (e.data("ptr").droppable.enabled = !1))
                : "hidden" === a[d].style.visibility &&
                  ((a[d].style.visibility = "visible"), (e.data("ptr").droppable.enabled = !0));
        }
    },
    recalculateElements: function () {
        for (
            var a, b = workspace.$el.find(".element"), c = workspace.$el.width(), d = $(".element").width(), a = 0;
            a < b.length;
            a++
        ) {
            var e = $(b.get(a));
        ...
        if (settings.data.saveElementsPositions) {
            ...
            if (null !== a)
                for (var b = 0, c = a.length; b < c; b++)
                    (game.progress.indexOf(a[b].id) === -1 && game.prime.indexOf(a[b].id) === -1) ||
                        workspace.add(a[b].id, a[b].position);
        }
    },
    elementsNamesVisibility: function () {
        for (var a, b = workspace.$el.find(".element"), c = null, d = 0, e = b.length; d < e; d++)
            (a = $(b[d])),
                (c = a.data("ptr")),
                settings.data.hideElementsNames
                    ? (c.hideElementName(), (a.find(".elementName")[0].style.opacity = 0))
                    : (c.removeHideElementName(), (a.find(".elementName")[0].style.opacity = 1));
    },
};
(WorkspaceBox.prototype.createElement = function (a) {
    var b = document.createElement("div");
    b.setAttribute("class", "element"),
        b.setAttribute("data-elementType", "workspaceBox"),
        b.setAttribute("data-elementId", this.id),
        (b.style.cssText = "position: absolute; left:" + a.left + "px; top:" + a.top + "px;");
    ...
    return (
        d.setAttribute("class", "elementName"),
        (d.textContent = bases.names[this.id]),
        settings.data.hideElementsNames && (d.style.opacity = 0),
        b.appendChild(c),
        b.appendChild(d),
        b
    );
    ...
    (WorkspaceBox.prototype.changeType = function () {
        this.$el[0].setAttribute("data-elementType", "workspaceBox"),
            settings.data.hideElementsNames && (this.$el.find(".elementName")[0].style.opacity = 0);
    }),
    (WorkspaceBox.prototype.init = function (a) {
        this.$el.data("ptr", this),
            ...
            (this.$el[0].style.zIndex = a ? a : "100"),
            bases.base[this.id].hasOwnProperty("hidden") && (this.$el[0].className += " hiddenElement"),
            this.initDraggable(),
            this.initDroppable(),
    ...
    (WorkspaceBox.prototype.initDraggable = function () {
        var a = this;
        (this.draggable = new Draggable(this.$el[0], { handle: "img", zIndex: 1e3, initialZIndex: 100 })),
            this.$el.on("dragStart", function (b) {
                settings.data.hideElementsNames &&
                    ((a.$el.find(".elementName")[0].style.opacity = 1), a.removeHideElementName()),
                    $(document).trigger("workspaceBoxDraggingStart"),
                    a.$el[0].parentNode.appendChild(a.$el[0]);
            }),
            this.$el.on("dragEnd", function (b, c) {
                var d = c.size.width / 2,
                    e = c.size.height / 2;
                return c.position.left < -d ||
                    c.position.left > workspace.$el.width() + d ||
                    c.position.top < -e ||
                    c.position.top > workspace.$el.height() - e
                    ? void workspace.del(a)
                    : (settings.data.hideElementsNames &&
                          (a.hideElementName(), (a.$el.find(".elementName")[0].style.opacity = 0)),
            ...
            this.$el.on("droppableDrop", function (b, c) {
                ...
                if (f.length > 0 && !workspace.alreadyCombined(e)) {
                    var g = workspace.calculateOffspringPosition(
                        [{ left: c.position.x, top: c.position.y }, a.$el.position()],
                        a.$el.width(),
                        f.length
                    );
                    for (i = 0; i < f.length; i++)
                        game.checkIfFinal(f[i]) && game.finalElements.push(f[i]), workspace.add(f[i], g[i]);
                    workspace.del(a),
                        "workspaceBox" === c.element.getAttribute("data-elementType")
                            ? workspace.del($(c.element).data("ptr"))
                            : $(c.helper).off().remove(),
                        $(document).trigger("childCreated", [f, e]);
                } else
                    "libraryBox" === c.element.getAttribute("data-elementType")
                        ? (new WorkspaceBox(c.helper),
                          workspace.alreadyCombined(e) || $(document).trigger("childCreationFail", [e]))
                        : workspace.alreadyCombined(e) || $(document).trigger("childCreationFail", [e]);
            });
    }),
    (WorkspaceBox.prototype.hideElementName = function () {
        for (var a = workspace.el.getElementsByClassName("elementName"), b = 0; b < a.length; b++)
            a[b].className = a[b].className.replace(" noTransition", "");
        var c = this;
    ...
    (WorkspaceBox.prototype.removeHideElementName = function () {
        ...
        for (var a = workspace.el.getElementsByClassName("elementName"), b = 0; b < a.length; b++)
            a[b].className += " noTransition";
    }),
    (WorkspaceBox.prototype.onOver = function () {
        (this.$el[0].style.opacity = 0.5),
            settings.data.hideElementsNames && (this.$el.find(".elementName")[0].style.opacity = 1);
    }),
        ...
        b.on(gestures.startEvents.join(" "), function (g) {
            if (!(("mousedown" === g.type && 1 !== g.which) || (null !== f && f !== a))) {
                if (
                    ...
                    return void b.on(gestures.events[g.type][0], function (c) {
                        1 === d &&
                            ((d = 0),
                            (f = null),
                            b.off(gestures.events[g.type][0]),
                            $(document).trigger("cloneWorkspaceBox", [c, a]));
                    });
                b.on(gestures.events[g.type][0], function (a) {
                    ...
                    $(document).on(gestures.startEvents.join(" "), function (a) {
                        a.target !== b[0] &&
                            ((d = 0), (f = null), clearTimeout(e), $(document).off(gestures.startEvents.join(" ")));
                    }),
                    d++;
            }
        }),
    ...
    $(document).ready(function () {
        game.checkOnline(),
            ...
            (window.onresize = function () {
                "undefined" != typeof iscrollAlphabeth &&
                    "undefined" != typeof iscrollLibrary &&
...
var initIScroll = function () {
    (iscrollLibrary = new IScroll("#outerLibrary", { mouseWheel: !0 })),
        iscrollLibrary.refresh(),
        iscrollLibrary.on("scrollStart", function () {
            $(document).trigger("libraryScrollStart");
        }),
        (iscrollAlphabeth = new IScroll("#alphabet", { mouseWheel: !0, click: !0 })),
