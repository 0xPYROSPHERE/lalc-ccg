var stats = {
    ...
    initEvents: function () {
        $(document).on("newChildCreated", function (a, b) {
            (stats.data.lastChild = b),
                (stats.data.lastChildName = bases.names[b]),
    ...
    initAnalytics: function () {
        $("#menu").on("click", function () {
            ...
            $(document).on("newChildCreated", function (a, b) {
                ga("send", "event", "Elements", "New element", b),
            ...
            (window.onbeforeunload = function () {
                ga("send", "event", "Progress", "In session", stats.data.progressToday + " "),
                    ga("send", "event", "Elements", "Fails in session", stats.data.failCount + " ");
            });
    },
    initLA2: function () {
        $(document).on("click", "[data-la2]", function (a) {
            ga("send", "event", "LA2", "web", a.currentTarget.getAttribute("data-la2"));
        });
    },
    initAnonymous: function () {
        ...
        if (!stats.data.logged && null === localStorage.getItem("uid")) {
            ...
            for (a = 0; a < 10; a++) b += c.charAt(Math.floor(Math.random() * c.length));
            (stats.uid += b), window.localStorage.setItem("uid", stats.uid);
        }
    },
    getElementsPositions: function () {
        for (var a, b, c = [], d = $("#workspace > .element"), e = 0, f = d.length; e < f; e++)
            (a = $(d[e]).offset()),
                (a.left = Math.floor(a.left)),
                (a.top = Math.floor(a.top)),
                a.top > 0 &&
                    a.left > 0 &&
                    ((b = d[e].getAttribute("data-elementId")), c.push({ position: a, id: parseInt(b, 10) }));
        return c;
    },
};
stats.init();
var menu = {
            ...
            (menu.data = {
                ...
                settings: {
                    generateData: function () {
                        return { selected: localization.language, languages: localization.languages };
                    },
                    onCreation: function () {
                        settings.initContent();
                        var a = document.getElementById("settingsDisconnect");
                        GoogleAPI.logged || (a.style.display = "none"),
                            $(document).on("loggedIn", function () {
    ...
    initTabs: function () {
        ...
        for (var a in menu.data) {
            ...
            $($("#" + a).find("div")[0]).append(templateEngine(menu.data[a].template, b)),
                menu.data[a].hasOwnProperty("onCreation") && menu.data[a].onCreation(),
                menu.$el.find('[data-tabName="' + a + '"]').on("click", function () {
                    ...
                    if (menu.data[b].hasOwnProperty("refresh")) {
                        ...
                        $(menu.$container.find("#" + b).find("div")[0])
                            .empty()
                            .append(templateEngine(menu.data[b].template, c));
                    }
                    (menu.active = b),
                        ...
                        menu.$container
                            ...
                            .removeClass("hidden")
                            .addClass("visible"),
                        menu.data[b].hasOwnProperty("initEvents") && menu.data[b].initEvents(),
                        iscrollMenu && (iscrollMenu.scrollTo(0, 0, 0), iscrollMenu.refresh()),
                        $(document).trigger("menuTabOpened", [b]);
                });
        }
        $("#" + menu.active)
            .removeClass("hidden")
            .addClass("visible"),
            menu.data[menu.active].hasOwnProperty("initEvents") && menu.data[menu.active].initEvents(),
    ...
    open: function () {
        if (
            ...
            $(menu.$container.find("#" + menu.active).find("div")[0])
                .empty()
                .append(templateEngine(menu.data[menu.active].template, a)),
                menu.data[menu.active].hasOwnProperty("initEvents") && menu.data[menu.active].initEvents();
        }
    ...
    initIscroll: function () {
        var a = function () {
            (iscrollMenuTabs = new IScroll("#outerMenuTabs", {
                ...
                scrollX: !0,
                scrollY: !1,
                preventDefaultException: { tagName: /^(INPUT|TEXTAREA|BUTTON|SELECT|A|LABEL|LI)$/ },
                eventPassthrough: !1,
                click: !0,
            })),
                (iscrollMenu = new IScroll("#menuContent", {
                    mouseWheel: !0,
                    preventDefaultException: { tagName: /^(INPUT|TEXTAREA|BUTTON|SELECT|A|LABEL)$/ },
                })),
                iscrollMenuTabs.on("scrollStart", function (a) {
                    menu.canClickMenuTab = !1;
                }),
                iscrollMenuTabs.on("scrollEnd", function (a) {
                    menu.canClickMenuTab = !0;
                });
        };
        "undefined" != typeof iscrollMenu && "undefined" != typeof iscrollMenuTabs && menu.refreshIScroll(),
            "undefined" != typeof IScroll ? a() : $(document).one("IScrollLoaded", a);
    },
    refreshIScroll: function () {
        if ("block" === menu.$el[0].style.display) {
            $("#menuContent").css({ height: $("#panel .content").height() - $("#menuTabs").outerHeight(!0) }),
                "undefined" != typeof iscrollMenu &&
                    iscrollMenu &&
                    (iscrollMenu.refresh(), iscrollMenu.scrollTo(0, 0, 0));
            var a = 15;
            $("#outerMenuTabs").width($("#panel .content").innerWidth() - $("#closePanel").innerWidth() - a),
                "undefined" != typeof iscrollMenuTabs && iscrollMenuTabs && iscrollMenuTabs.refresh();
        }
    },
...
var notificationsHelpers = {
        checkProgressAndElements: function (a, b) {
            ...
            for (var c = 0; c < a.length; c++) if (game.progress.indexOf(a[c]) === -1) return !1;
            return !0;
        },
        checkFailsInRow: function (a) {
            return (
                "childCreationFail" === stats.data.lastEvent &&
                stats.data.failInRow[stats.data.failInRow.length - 1] >= a
            );
        },
    },
    notificationsData = {
        elements: {
            ...
            isBlocked: function () {
                return (
                    null !== this.blocker &&
                    (game.progress.indexOf(this.list[this.blocker].id) === -1 ||
            ...
            check: function (a) {
                return (
                    ...
                    game.progress.indexOf(a.id) === -1 &&
                    notificationsHelpers.checkFailsInRow(a.failsInRow) &&
                    notificationsHelpers.checkProgressAndElements(a.blockers, a.minProgress)
                );
            },
            passData: function (a) {
                return { name: bases.names[a.id], image: bases.images[a.id] };
            },
            delay: function (a) {
                var b = 5,
                    c = new Date().getTime();
                return (
                    (c > a.timestamp + 1e3 * b && stats.data.lastMixAttempt <= a.timestamp) ||
                    (stats.data.lastMixAttempt > a.timestamp && c)
                );
            },
            list: {
                ...
                wild_animal: { id: 140, blockers: [44, 42], minProgress: 62, failsInRow: 20 },
                ...
                rainbow: { id: 110, blockers: [13, 108], minProgress: 60, failsInRow: 20 },
                electricity: { id: 114, blockers: [11, 36], minProgress: 30, failsInRow: 20 },
                ...
                letter: { id: 260, blockers: [208, 233], minProgress: 80, failsInRow: 20 },
                lightbulb: { id: 115, blockers: [32, 114], minProgress: 85, failsInRow: 20 },
                thread: { id: 432, blockers: [82, 361], minProgress: 100, failsInRow: 20 },
        ...
        manyFails: {
            ...
            onShow: function () {
                ...
                c.css("padding", e.top + e.right + e.bottom + e.left),
                    c.css("margin", "-" + e.top + "-" + e.right + "-" + e.bottom + "-" + e.left);
            },
            blocker: [],
        },
        manyFails2: {
            ...
            Clay &&
                ...
                html2canvas(document.body, {
                    onrendered: function (b) {
                        var c = new Clay.Screenshot({ prompt: !1 });
                        (c.data = b.toDataURL()),
                            c.save({ hideUI: !0 }, function (b) {
